...
fn write_callback(
    ...
    const second_per_frame = 1.0 / float_sample_rate;
    ...
    while (frames_left > 0) {
        ...
        while (frame < frame_count) : (frame + 1) {
            ...
            const sample = std.math.sin((seconds_offset + float_frame * second_per_frame) * radians_per_second);
            {
                var channel: usize = 0;
                while (channel < @as(usize, @intCast(layout.channel_cout))) : (channel += 1) {
        ...
        seconds_offset += second_per_frame * float_frame_count;
...
pub fn main() !void {
    ...
    defer c.soudio_destroy(soundio);
    ...
    defer c.soudio_device_unref(device);
    ...
    outstream.*.format = c.SoundIoFormat32NE;
